/** Узгоджено з src/data/balance та calculatePhysical/MagicDamage (PvE). */
...
export function mobDefenseFromLevel(level: number): { pDef: number; mDef: number } {
  const lv = Math.max(1, Math.floor(level));
  const rawP = Math.round(lv * 12);
  const rawM = Math.round(lv * 10);
  return {
...
export function resolveMobMaxHp(args: {
  registryLevel: number;
  isRaidBoss: boolean;
  clientMobMaxHp: number;
}): number {
  const lv = Math.max(1, Math.floor(args.registryLevel));
  const base = 50 + lv * 80;
  const defaultHp = args.isRaidBoss ? base : Math.round(base * MOB_HP_MULTIPLIER);
  const c = Math.floor(Number(args.clientMobMaxHp) || 0);
  const minHp = Math.max(1, Math.floor(defaultHp * 0.35));
  const maxHp = Math.max(minHp + 1, Math.ceil(defaultHp * 2.2));
  if (c > 0) return Math.min(maxHp, Math.max(minHp, c));
  return defaultHp;
}
...
function elementMultiplier(
  ...
  target: { fireResist: number; waterResist: number; windResist: number; earthResist: number; holyResist: number; darkResist: number }
): number {
  if (!element) return 1;
  const attackBonus =
    element === "fire"
