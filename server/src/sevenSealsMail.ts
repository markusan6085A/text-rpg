/**
 * @deprecated Логіка перенесена в `sevenSealsFinalize.ts` (субота 22:00 Варшава + нагороди).
 * Файл залишено для історії; не підключайте до index.
 */
...
/** Точні числа з’являються в грі після авто-нарахування на екрані «Персонаж» (сповіщення). */
const WINNER_MESSAGES: Record<number, string> = {
  1: `Вітаємо! Ви посіли 1 місце в івенті «7 Печатей».

Нагорода нараховується автоматично, коли ви відкриєте екран «Персонаж». Окремо нічого забирати в інвентарі не потрібно — після нарахування з’явиться сповіщення з точними значеннями: фіз. атака, маг. атака, фіз. захист, маг. захист та Coin of Luck (CoL).`,
...
export async function runSevenSealsMailJob(
  ...
  try {
    ...
    const subject = "7 Печатей — вітання!";
