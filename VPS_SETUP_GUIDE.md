# 🖥️ Детальний гайд: Налаштування VPS для text-rpg

## 📋 Зміст

1. [Вибір VPS провайдера](#1-вибір-vps-провайдера)
2. [Створення сервера](#2-створення-сервера)
3. [Підключення по SSH](#3-підключення-по-ssh)
4. [Початкове налаштування сервера](#4-початкове-налаштування-сервера)
5. [Встановлення Node.js](#5-встановлення-nodejs)
6. [Встановлення Docker та PostgreSQL](#6-встановлення-docker-та-postgresql)
7. [Встановлення PM2](#7-встановлення-pm2)
8. [Встановлення nginx](#8-встановлення-nginx)
9. [Налаштування SSL (Let's Encrypt)](#9-налаштування-ssl-lets-encrypt)
10. [Налаштування деплою з GitHub](#10-налаштування-деплою-з-github)
11. [Налаштування Environment Variables](#11-налаштування-environment-variables)
12. [Встановлення залежностей, міграції та збірка](#12-встановлення-залежностей-міграції-та-збірка)
13. [Тестування сервера](#13-тестування-сервера)
14. [Перший деплой з PM2](#14-перший-деплой-з-pm2)
15. [Налаштування автозапуску](#15-налаштування-автозапуску)
16. [Налаштування моніторингу (опціонально)](#16-налаштування-моніторингу-опціонально)
17. [Налаштування firewall](#17-налаштування-firewall)
18. [Troubleshooting](#18-troubleshooting)

---

## 1. Вибір VPS провайдера

### Рекомендовані провайдери:

#### ⭐ **Hetzner** (Німеччина) - Найкраще співвідношення ціна/якість
- **Ціна:** €4-6/місяць
- **Характеристики:** 2-4 GB RAM, 1-2 CPU, 20-40 GB SSD
- **Плюси:** Швидкий, стабільний, дешевий
- **Мінуси:** Тільки Європа
- **Сайт:** https://www.hetzner.com/cloud

#### **DigitalOcean** (США, глобально)
- **Ціна:** $6-12/місяць
- **Характеристики:** 1-2 GB RAM, 1 CPU, 25 GB SSD
- **Плюси:** Добра документація, багато регіонів
- **Сайт:** https://www.digitalocean.com

#### **Vultr** (глобально)
- **Ціна:** $6-12/місяць
- **Характеристики:** 1-2 GB RAM, 1 CPU, 25 GB SSD
- **Плюси:** Багато регіонів, швидкий
- **Сайт:** https://www.vultr.com

#### **Contabo** (Німеччина) - Найдешевший
- **Ціна:** €4-5/місяць
- **Характеристики:** 4 GB RAM, 2 CPU, 50 GB SSD
- **Плюси:** Дуже дешево, багато ресурсів
- **Мінуси:** Може бути повільніше під навантаженням
- **Сайт:** https://www.contabo.com

### Мінімальні вимоги для text-rpg:
- **RAM:** 1-2 GB (рекомендовано 2 GB)
- **CPU:** 1-2 ядра
- **Диск:** 20-40 GB SSD
- **ОС:** Ubuntu 22.04 LTS (рекомендовано)

---

## 2. Створення сервера

### Крок 1: Реєстрація
1. Зареєструйтеся на вибраному провайдері
2. Підтвердіть email

### Крок 2: Створення VPS
1. Перейдіть в панель управління
2. Натисніть **"Create Server"** або **"New Server"**
3. Оберіть:
   - **ОС:** Ubuntu 22.04 LTS
   - **Регіон:** Найближчий до ваших користувачів (Європа для України)
   - **Розмір:** 2 GB RAM, 1-2 CPU, 20-40 GB SSD
   - **SSH Key:** Додайте свій SSH ключ (або створить пізніше)
4. Натисніть **"Create"** або **"Deploy"**

### Крок 3: Отримати IP та пароль
- Провайдер покаже **IP адресу** сервера
- Якщо не додали SSH ключ, отримаєте **root пароль** (збережіть!)

---

## 3. Підключення по SSH

### Windows (PowerShell або Git Bash):

```bash
# Підключення
ssh root@YOUR_IP_ADDRESS

# Або якщо використовуєте SSH ключ
ssh -i path/to/your/key.pem root@YOUR_IP_ADDRESS
```

**Приклад:**
```bash
ssh root@123.45.67.89
```

### Якщо вперше підключаєтесь:
- Система запитає: `Are you sure you want to continue connecting (yes/no)?`
- Введіть: `yes`
- Введіть пароль (якщо не використовуєте SSH ключ)

### Якщо не можете підключитись:
- Перевірте, чи правильно скопійовано IP
- Перевірте firewall на провайдера (має бути відкритий порт 22)
- Спробуйте через інший термінал (Git Bash, WSL)

---

## 4. Початкове налаштування сервера

### Крок 1: Оновити систему

```bash
# Оновити список пакетів
apt update

# Оновити встановлені пакети
apt upgrade -y

# Перезавантажити (якщо потрібно)
reboot
```

### Крок 2: Створити користувача (замість root)

```bash
# Створити нового користувача
adduser deploy

# Додати в групу sudo (для адміністративних прав)
usermod -aG sudo deploy

# Переключитись на нового користувача
su - deploy
```

### Крок 3: Налаштувати SSH ключ (опціонально, але рекомендовано)

**На вашому комп'ютері (Windows):**

```bash
# Генерувати SSH ключ (якщо ще немає)
ssh-keygen -t ed25519 -C "your_email@example.com"

# Скопіювати публічний ключ
cat ~/.ssh/id_ed25519.pub
# Скопіюйте весь вивід
```

**На сервері:**

```bash
# Створити директорію для SSH ключів
mkdir -p ~/.ssh
chmod 700 ~/.ssh

# Відкрити файл для редагування
nano ~/.ssh/authorized_keys

# Вставити ваш публічний ключ (Ctrl+Shift+V)
# Зберегти: Ctrl+O, Enter, Ctrl+X

# Встановити правильні права
chmod 600 ~/.ssh/authorized_keys
```

### Крок 4: Вимкнути вхід по паролю (безпека)

```bash
# Відкрити конфігурацію SSH
sudo nano /etc/ssh/sshd_config

# Знайти рядок: #PasswordAuthentication yes
# Змінити на: PasswordAuthentication no

# Зберегти: Ctrl+O, Enter, Ctrl+X

# Перезапустити SSH
sudo systemctl restart sshd
```

---

## 5. Встановлення Node.js

### Встановити Node.js 20.x LTS (рекомендовано):

```bash
# Оновити систему
sudo apt update

# Встановити curl (якщо немає)
sudo apt install -y curl

# Додати NodeSource репозиторій
curl -fsSL https://deb.nodesource.com/setup_20.x | sudo -E bash -

# Встановити Node.js
sudo apt install -y nodejs

# Перевірити версію
node --version  # Має показати v20.x.x
npm --version   # Має показати версію npm
```

### Встановити глобальні пакети:

```bash
# Встановити PM2 глобально (потрібно для керування процесами)
sudo npm install -g pm2

# Перевірити
pm2 --version
```

---

## 6. Встановлення Docker та PostgreSQL

### Варіанти використання БД:

**Варіант А: Локальна PostgreSQL на VPS (рекомендовано для VPS)**
- ✅ Швидше (немає мережевих затримок)
- ✅ Безкоштовно (не потрібен Supabase)
- ✅ Повний контроль
- ⚠️ Потрібно налаштувати backup

**Варіант Б: Supabase (як на Railway)**
- ✅ Автоматичний backup
- ✅ Готовий до використання
- ⚠️ Мережеві затримки
- ⚠️ Обмеження на з'єднання

**Рекомендація:** Для VPS використовуйте локальну PostgreSQL через Docker.

---

### Крок 1: Встановити Docker

```bash
# Оновити систему
sudo apt update

# Встановити необхідні пакети
sudo apt install -y ca-certificates curl gnupg lsb-release

# Додати Docker GPG ключ
sudo mkdir -p /etc/apt/keyrings
curl -fsSL https://download.docker.com/linux/ubuntu/gpg | sudo gpg --dearmor -o /etc/apt/keyrings/docker.gpg

# Додати Docker репозиторій
echo \
  "deb [arch=$(dpkg --print-architecture) signed-by=/etc/apt/keyrings/docker.gpg] https://download.docker.com/linux/ubuntu \
  $(lsb_release -cs) stable" | sudo tee /etc/apt/sources.list.d/docker.list > /dev/null

# Встановити Docker
sudo apt update
sudo apt install -y docker-ce docker-ce-cli containerd.io docker-buildx-plugin docker-compose-plugin

# Додати користувача в групу docker (щоб не використовувати sudo)
sudo usermod -aG docker $USER

# Перевірити встановлення
docker --version
docker compose version
```

**Важливо:** Після додавання користувача в групу docker, потрібно вийти і знову увійти в SSH:

```bash
# Вийти з SSH
exit

# Підключитись знову
ssh deploy@YOUR_IP
```

### Крок 2: Створити директорію для проекту

```bash
# Створити директорію
sudo mkdir -p /opt/text-rpg
sudo chown $USER:$USER /opt/text-rpg
cd /opt/text-rpg
```

### Крок 3: Створити docker-compose.yml

**Варіант А: Скопіювати з проекту (якщо вже клонували репозиторій):**

```bash
# Якщо вже клонували репозиторій
cp ~/text-rpg/docker-compose.yml /opt/text-rpg/
cd /opt/text-rpg
```

**Варіант Б: Створити вручну:**

```bash
# Створити docker-compose.yml
nano docker-compose.yml
```

**Вставити наступну конфігурацію:**

```yaml
services:
  db:
    image: postgres:16
    restart: always
    environment:
      POSTGRES_DB: game
      POSTGRES_USER: game
      POSTGRES_PASSWORD: change_me_strong
    volumes:
      - pgdata:/var/lib/postgresql/data
    ports:
      - "127.0.0.1:5432:5432"
    healthcheck:
      test: ["CMD-SHELL", "pg_isready -U game -d game"]
      interval: 10s
      timeout: 5s
      retries: 5

volumes:
  pgdata:
```

**Замінити:**
- `change_me_strong` на сильний пароль (мінімум 16 символів)

**Зберегти:** `Ctrl+O`, `Enter`, `Ctrl+X`

### Крок 4: Запустити PostgreSQL

```bash
# Запустити PostgreSQL
docker compose up -d

# Перевірити статус контейнерів
docker ps
```

**Має показати контейнер `db` зі статусом `Up`**

```bash
# Перевірити логи (опціонально)
docker compose logs db
```

**Має показати:** `database system is ready to accept connections`

### Крок 5: Перевірити підключення

```bash
# Встановити PostgreSQL client (для тестування)
sudo apt install -y postgresql-client

# Підключитись до БД
psql -h localhost -U game -d game
```

**Ввести пароль** (той, що вказали в docker-compose.yml)

**Якщо підключення успішне, ви побачите:**
```
game=#
```

**Вийти:** `\q` або `Ctrl+D`

### Крок 6: Налаштувати автозапуск Docker Compose

Docker Compose автоматично запускається при перезавантаженні (через `restart: always`), але перевіримо:

```bash
# Перезавантажити сервер (опціонально, для тесту)
sudo reboot

# Після перезавантаження перевірити
docker compose ps
```

---

## 7. Встановлення PM2

PM2 вже встановлено на попередньому кроці, але налаштуємо його:

```bash
# Налаштувати PM2 для автозапуску при перезавантаженні
pm2 startup

# Скопіювати команду, яку покаже PM2, і виконати її
# Приклад: sudo env PATH=$PATH:/usr/bin pm2 startup systemd -u deploy --hp /home/deploy

# Перевірити статус
pm2 status
```

---

## 8. Встановлення nginx

### Встановити nginx:

```bash
# Встановити nginx
sudo apt install -y nginx

# Запустити nginx
sudo systemctl start nginx

# Додати в автозапуск
sudo systemctl enable nginx

# Перевірити статус
sudo systemctl status nginx
```

### Налаштувати nginx для text-rpg:

```bash
# Створити конфігурацію для text-rpg
sudo nano /etc/nginx/sites-available/text-rpg
```

**Вставити наступну конфігурацію:**

```nginx
server {
    listen 80;
    server_name YOUR_DOMAIN_OR_IP;

    # Логування
    access_log /var/log/nginx/text-rpg-access.log;
    error_log /var/log/nginx/text-rpg-error.log;

    # Максимальний розмір завантаження (для файлів)
    client_max_body_size 10M;

    # Проксування на Node.js сервер (який працює на порту 3000)
    location / {
        proxy_pass http://localhost:3000;
        proxy_http_version 1.1;
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection 'upgrade';
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
        proxy_cache_bypass $http_upgrade;
        
        # Таймаути
        proxy_connect_timeout 60s;
        proxy_send_timeout 60s;
        proxy_read_timeout 60s;
    }

    # Health check endpoint (опціонально)
    location /health {
        proxy_pass http://localhost:3000/health;
        access_log off;
    }
}
```

**Замінити:**
- `YOUR_DOMAIN_OR_IP` на ваш домен або IP адресу

**Зберегти:** `Ctrl+O`, `Enter`, `Ctrl+X`

### Активувати конфігурацію:

```bash
# Створити символічне посилання
sudo ln -s /etc/nginx/sites-available/text-rpg /etc/nginx/sites-enabled/

# Видалити дефолтну конфігурацію (якщо не потрібна)
sudo rm /etc/nginx/sites-enabled/default

# Перевірити конфігурацію nginx
sudo nginx -t

# Якщо все ОК, перезавантажити nginx
sudo systemctl reload nginx
```

---

## 9. Налаштування SSL (Let's Encrypt)

### Встановити Certbot:

```bash
# Встановити Certbot
sudo apt install -y certbot python3-certbot-nginx

# Перевірити
certbot --version
```

### Отримати SSL сертифікат:

**Якщо маєте домен:**

```bash
# Отримати сертифікат (Certbot автоматично налаштує nginx)
sudo certbot --nginx -d your-domain.com -d www.your-domain.com

# Відповісти на питання:
# - Email: ваш email
# - Terms: A (Agree)
# - Share email: N (No) або Y (Yes)
```

**Якщо НЕ маєте домен (тільки IP):**
- SSL сертифікат можна отримати тільки для домену
- Можна використовувати безкоштовний домен (Freenom, No-IP) або купити ($1-2/рік)
- Або використовувати без SSL (не рекомендовано для production)

### Автоматичне оновлення сертифікату:

```bash
# Перевірити, чи налаштовано автоматичне оновлення
sudo certbot renew --dry-run
```

Certbot автоматично налаштує cron job для оновлення сертифікатів.

---

## 10. Налаштування деплою з GitHub

### Крок 1: Створити директорію для проекту:

```bash
# Створити директорію для проекту (рекомендовано /opt/text-rpg)
sudo mkdir -p /opt/text-rpg
sudo chown $USER:$USER /opt/text-rpg
cd /opt/text-rpg
```

### Крок 2: Клонувати репозиторій:

```bash
# Клонувати репозиторій (замініть на ваш)
git clone https://github.com/your-username/text-rpg.git .

# Або якщо приватний репозиторій, використати SSH:
# git clone git@github.com:your-username/text-rpg.git .
```

### Крок 3: Скопіювати docker-compose.yml (якщо ще не зроблено):

```bash
# Якщо docker-compose.yml ще не в /opt/text-rpg
cp docker-compose.yml /opt/text-rpg/ 2>/dev/null || echo "docker-compose.yml вже на місці"
```

### Крок 4: Створити скрипт для деплою:

```bash
# Створити скрипт для деплою
nano ~/deploy-text-rpg.sh
```

**Вставити:**

```bash
#!/bin/bash

# Кольори для виводу
GREEN='\033[0;32m'
RED='\033[0;31m'
NC='\033[0m' # No Color

echo -e "${GREEN}🚀 Початок деплою text-rpg...${NC}"

# Перейти в директорію проекту
cd ~/text-rpg

# Отримати останні зміни з GitHub
echo -e "${GREEN}📥 Отримання змін з GitHub...${NC}"
git pull origin main || git pull origin master

# Перейти в директорію server
cd ~/text-rpg/server

# Встановити залежності
echo -e "${GREEN}📦 Встановлення залежностей...${NC}"
npm install

# Згенерувати Prisma Client
echo -e "${GREEN}🔧 Генерація Prisma Client...${NC}"
npm run prisma:generate

# Запустити міграції (якщо потрібно)
echo -e "${GREEN}🗄️ Запуск міграцій...${NC}"
npm run prisma:migrate:deploy || echo -e "${RED}⚠️ Помилка міграцій (може бути нормально, якщо вже виконано)${NC}"

# Зібрати проект
echo -e "${GREEN}🔨 Збірка проекту...${NC}"
npm run build

# Перезапустити PM2
echo -e "${GREEN}🔄 Перезапуск PM2...${NC}"
pm2 restart text-rpg || pm2 start dist/index.js --name text-rpg

echo -e "${GREEN}✅ Деплой завершено!${NC}"
pm2 status
```

**Зберегти:** `Ctrl+O`, `Enter`, `Ctrl+X`

**Зробити виконуваним:**

```bash
chmod +x ~/deploy-text-rpg.sh
```

---

## 11. Налаштування Environment Variables

### Створити .env файл:

```bash
# Перейти в директорію server
cd /opt/text-rpg/server

# Створити .env файл
nano .env
```

### Варіант А: Локальна PostgreSQL (рекомендовано для VPS)

**Вставити (замініть `change_me_strong` на пароль з docker-compose.yml):**

```env
NODE_ENV=production
PORT=3000

DATABASE_URL="postgresql://game:change_me_strong@127.0.0.1:5432/game?schema=public"
JWT_SECRET="change_this_to_a_long_random_secret_64_chars_min"
```

**Важливо:**
- Замініть `change_me_strong` на пароль з `/opt/text-rpg/docker-compose.yml`
- Замініть `change_this_to_a_long_random_secret_64_chars_min` на сильний випадковий рядок (мінімум 64 символи)

**Формат DATABASE_URL для локальної БД:**
```
postgresql://USER:PASSWORD@localhost:5432/DATABASE
```

Де:
- `USER` = `game` (з docker-compose.yml)
- `PASSWORD` = пароль з docker-compose.yml (замініть `change_me_strong`)
- `DATABASE` = `game` (з docker-compose.yml)

### Варіант Б: Supabase (якщо використовуєте Supabase)

**Вставити (замініть на ваші значення):**

```env
# База даних (Supabase pooler для runtime)
DATABASE_URL=postgresql://postgres.hstwsloooubalvpwasst:YOUR_PASSWORD@aws-1-eu-west-1.pooler.supabase.com:6543/postgres?pgbouncer=true&connection_limit=1

# JWT секрет (згенеруйте новий!)
JWT_SECRET=your-super-secret-jwt-key-minimum-32-characters-long

# Порт (за замовчуванням 3000)
PORT=3000

# Оточення
NODE_ENV=production
```

**Згенерувати JWT_SECRET (мінімум 64 символи, випадкові):**

```bash
# На сервері (рекомендований спосіб)
openssl rand -hex 64
```

**Або через Node.js:**
```bash
node -e "console.log(require('crypto').randomBytes(64).toString('hex'))"
```

**Скопіювати результат і вставити в .env як JWT_SECRET (в лапках):**
```env
JWT_SECRET="скопійований_результат_тут"
```

**Зберегти:** `Ctrl+O`, `Enter`, `Ctrl+X`

---

## 12. Встановлення залежностей, міграції та збірка

### Крок 1: Встановити залежності та налаштувати Prisma:

```bash
# Перейти в директорію server
cd /opt/text-rpg/server

# Встановити залежності (npm ci для production - використовує package-lock.json)
npm ci

# Згенерувати Prisma Client
npm run prisma:generate

# Запустити міграції бази даних
npm run prisma:migrate:deploy

# Зібрати проект
npm run build
```

**Якщо міграції успішні - БД налаштовано правильно!**

---

## 13. Тестування сервера

### Крок 1: Запустити сервер для перевірки:

```bash
# Переконатись, що знаходимось в правильній директорії
cd /opt/text-rpg/server

# Запустити сервер
npm run start
```

**Якщо все ОК - сервер почне слухати на порту 3000:**
```
Server started on http://0.0.0.0:3000
```

**Зупинити сервер:** `Ctrl+C`

### Крок 2: Відкрити порт 3000 для тестування (опціонально):

```bash
# Дозволити порт 3000 через firewall
sudo ufw allow 3000/tcp

# Перевірити статус firewall
sudo ufw status
```

**Тепер можна перевірити з вашого ПК:**
- `http://YOUR_IP_ADDRESS:3000`
- `http://YOUR_IP_ADDRESS:3000/health` (якщо є health endpoint)

**Після тестування закрийте порт 3000 (безпека):**
```bash
sudo ufw delete allow 3000/tcp
```

---

## 14. Перший деплой з PM2

### Крок 1: Запустити сервер через PM2:

```bash
# Переконатись, що знаходимось в правильній директорії
cd /opt/text-rpg/server

# Запустити сервер через PM2 (краще використовувати dist/index.js напряму)
pm2 start dist/index.js --name text-rpg-api

# Перевірити статус
pm2 status
```

**Має показати `text-rpg-api` зі статусом `online`**

**Чому `dist/index.js` замість `npm run start`:**
- Менше обгорток
- Простіші логи
- Швидший старт

### Крок 2: Переглянути логи:

```bash
# Переглянути логи PM2
pm2 logs text-rpg

# Або останні 50 рядків
pm2 logs text-rpg --lines 50
```

**Має показати:** `Server started on http://0.0.0.0:3000`

### Крок 3: Зберегти конфігурацію PM2:

```bash
# Зберегти поточний список процесів для автозапуску
pm2 save

# Налаштувати автозапуск при перезавантаженні системи
pm2 startup
# Виконати команду, яку покаже PM2 (зазвичай з sudo)
```

**Тепер сервер буде автоматично запускатись при перезавантаженні!**

### Крок 4: Перевірити, що API працює:

```bash
# Перевірити, чи слухає порт 3000
ss -lntp | grep :3000 || netstat -lntp | grep :3000

# Перевірити health endpoint
curl -i http://127.0.0.1:3000/health
```

**Має показати:** `HTTP/1.1 200 OK` та `{"status":"ok"}`

### Крок 4: Перевірити через браузер:

Відкрити в браузері:
- `http://YOUR_IP_ADDRESS/health` (через nginx, якщо налаштовано)
- Або `http://YOUR_IP_ADDRESS:3000/health` (якщо порт 3000 відкритий)
- Має показати: `{"status":"ok"}`

---

## 15. Налаштування автозапуску

### PM2 вже налаштовано на кроці 6, але перевіримо:

```bash
# Перевірити, чи налаштовано автозапуск
pm2 startup

# Якщо покаже команду, виконайте її
# Приклад: sudo env PATH=$PATH:/usr/bin pm2 startup systemd -u deploy --hp /home/deploy

# Зберегти поточний список процесів
pm2 save
```

**Тепер при перезавантаженні сервера PM2 автоматично запустить text-rpg!**

---

## 16. Налаштування моніторингу (опціонально)

### PM2 Monitoring (безкоштовно):

```bash
# Встановити PM2 Plus (опціонально, потребує реєстрації)
pm2 link

# Або використовувати локальний моніторинг
pm2 monit
```

### Налаштувати логи:

```bash
# Налаштувати ротацію логів
pm2 install pm2-logrotate

# Налаштувати параметри
pm2 set pm2-logrotate:max_size 10M
pm2 set pm2-logrotate:retain 7
pm2 set pm2-logrotate:compress true
```

---

## 17. Налаштування firewall

### Встановити UFW (Uncomplicated Firewall):

```bash
# Встановити UFW
sudo apt install -y ufw

# Дозволити SSH (ВАЖЛИВО! Інакше заблокуєте себе)
sudo ufw allow 22/tcp

# Дозволити HTTP
sudo ufw allow 80/tcp

# Дозволити HTTPS
sudo ufw allow 443/tcp

# Увімкнути firewall
sudo ufw enable

# Перевірити статус
sudo ufw status
```

---

## 18. Troubleshooting

### Проблема: Сервер не запускається

```bash
# Перевірити логи PM2
pm2 logs text-rpg --lines 50

# Перевірити, чи є помилки в .env
cd ~/text-rpg/server
cat .env

# Перевірити, чи правильно зібрано проект
ls -la dist/
```

### Проблема: Не можу підключитись по HTTP/HTTPS

```bash
# Перевірити, чи працює nginx
sudo systemctl status nginx

# Перевірити, чи працює Node.js сервер
curl http://localhost:3000/health

# Перевірити конфігурацію nginx
sudo nginx -t

# Перевірити логи nginx
sudo tail -f /var/log/nginx/text-rpg-error.log
```

### Проблема: Помилка підключення до БД

**Якщо використовуєте локальну PostgreSQL:**

```bash
# Перевірити, чи працює PostgreSQL
docker compose ps
cd /opt/text-rpg
docker compose logs db

# Перевірити DATABASE_URL
cd ~/text-rpg/server
cat .env | grep DATABASE_URL

# Перевірити підключення вручну
psql -h localhost -U game -d game
```

**Якщо використовуєте Supabase:**

```bash
# Перевірити DATABASE_URL
cd ~/text-rpg/server
cat .env | grep DATABASE_URL

# Перевірити, чи правильно вказано pooler URL
# Має бути порт 6543, не 5432!
```

### Проблема: PM2 не запускається після перезавантаження

```bash
# Перевірити автозапуск
pm2 startup

# Зберегти список процесів
pm2 save

# Перевірити статус
pm2 status
```

### Проблема: SSL сертифікат не оновлюється

```bash
# Перевірити статус Certbot
sudo certbot certificates

# Оновити вручну
sudo certbot renew

# Перевірити cron job
sudo systemctl status certbot.timer
```

---

## ✅ Перевірка після налаштування

### Чеклист:

- [ ] PostgreSQL працює: `docker ps` показує контейнер `db` зі статусом `Up`
- [ ] Порт 5432 закритий назовні: `ss -lntp | grep :5432` показує `127.0.0.1:5432` (не `0.0.0.0:5432`)
- [ ] Сервер запускається: `pm2 status` показує `text-rpg-api` online
- [ ] Health endpoint працює: `curl http://127.0.0.1:3000/health` → `{"status":"ok"}`
- [ ] Nginx працює: `sudo systemctl status nginx` → active
- [ ] SSL працює (якщо налаштовано): `https://your-domain.com/health` → `{"status":"ok"}`
- [ ] Firewall налаштовано: `sudo ufw status` → активний (порти 22, 80, 443 відкриті, 3000 закритий)
- [ ] Автозапуск працює: `pm2 save` виконано, `pm2 startup` налаштовано
- [ ] JWT_SECRET встановлено (мінімум 64 символи): `cat /opt/text-rpg/server/.env | grep JWT_SECRET`

---

## 🚀 Оновлення проекту (після налаштування)

### Кожного разу, коли робите зміни:

```bash
# На сервері
~/deploy-text-rpg.sh
```

**Або вручну:**

```bash
cd ~/text-rpg
git pull
cd server
npm install
npm run prisma:generate
npm run build
pm2 restart text-rpg
```

---

## 📝 Корисні команди

```bash
# Переглянути логи PM2
pm2 logs text-rpg-api

# Перезапустити сервер
pm2 restart text-rpg-api

# Зупинити сервер
pm2 stop text-rpg-api

# Запустити сервер
pm2 start text-rpg-api

# Перезавантажити nginx
sudo systemctl reload nginx

# Переглянути логи nginx
sudo tail -f /var/log/nginx/text-rpg-error.log

# Перезавантажити сервер
sudo reboot
```

---

## 🎯 Наступні кроки

1. ✅ Налаштувати домен (якщо ще не налаштовано)
2. ✅ Налаштувати backup бази даних (Supabase має автоматичний backup)
3. ✅ Налаштувати моніторинг (PM2 Plus або інший сервіс)
4. ✅ Оновити VITE_API_URL на Vercel (якщо frontend на Vercel)

---

**Готово! 🎉 Ваш text-rpg тепер працює на VPS без cold start!**
