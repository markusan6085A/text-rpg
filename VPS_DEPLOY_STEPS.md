# 🚀 Швидкий деплой на VPS

## ⚡ Автоматичний спосіб (рекомендовано)

```bash
# На VPS сервері
cd /opt/text-rpg
git pull
chmod +x server/deploy-vps.sh
./server/deploy-vps.sh
```

Скрипт автоматично виконає всі кроки 2-5.

<hr style="border: none; border-top: 2px dotted #C9B36B; margin: 20px 0;">

## 📝 Ручний спосіб (крок за кроком)

## Крок 1: ПК (Cursor) - Запушити зміни в GitHub

У терміналі Cursor в корені проєкту (там де package.json, server/, src/):

```bash
git status
git add -A
git commit -m "fix: build and deploy"
git push
```

**Якщо `git commit` скаже "nothing to commit"** — значить нічого реально не зберіг/не змінив у цій папці.

<hr style="border: none; border-top: 2px dotted #C9B36B; margin: 20px 0;">

## Крок 2: VPS - Забрати зміни з GitHub

Заходиш на сервер:

```bash
ssh root@116.203.243.128
```

Далі:

```bash
cd /opt/text-rpg
git pull
```

**Перевір що підтягнуло:**

```bash
git log -1 --oneline
```

<hr style="border: none; border-top: 2px dotted #C9B36B; margin: 20px 0;">

## Крок 3: VPS - Перезібрати backend (після pull)

**Як на Railway - автоматично міграції:**

```bash
cd /opt/text-rpg/server
npm ci
npm run prisma:generate
npm run prisma:migrate:deploy
npm run build
```

**Якщо `npm run build` пройшов** — далі запускаємо.

<hr style="border: none; border-top: 2px dotted #C9B36B; margin: 20px 0;">

## Крок 4: VPS - Запуск (тимчасово перевірка)

```bash
cd /opt/text-rpg/server
node dist/index.js
```

**В іншому вікні (або Ctrl+C і далі) перевірка:**

```bash
curl -i http://127.0.0.1:3000/health
```

**Якщо відповідає** — зупини `Ctrl+C` і запускаємо через PM2.

<hr style="border: none; border-top: 2px dotted #C9B36B; margin: 20px 0;">

## Крок 5: VPS - Запуск через PM2 (постійно)

```bash
cd /opt/text-rpg/server

# Зупинити старий процес (якщо існує)
pm2 delete text-rpg-api 2>/dev/null || true

# Запустити новий
pm2 start dist/index.js --name text-rpg-api
pm2 save
pm2 startup
```

**Команду, яку покаже `pm2 startup`, виконай 1 раз (зазвичай з sudo).**

**Статус:**

```bash
pm2 status
pm2 logs text-rpg-api --lines 80
```

<hr style="border: none; border-top: 2px dotted #C9B36B; margin: 20px 0;">

## Швидка перевірка після деплою

```bash
# 1. Статус контейнерів
docker ps

# 2. Статус PM2
pm2 status

# 3. Health check
curl -i http://127.0.0.1:3000/health

# 4. Перевірка через nginx (якщо налаштовано)
curl -i http://127.0.0.1/health
```

<hr style="border: none; border-top: 2px dotted #C9B36B; margin: 20px 0;">

## Troubleshooting

### Проблема: git pull не працює

```bash
# Перевірити чи є зміни
git status

# Якщо є конфлікти
git stash
git pull
git stash pop
```

### Проблема: npm run build не працює

```bash
# Перевірити помилки
npm run build 2>&1 | tee build.log

# Перевірити .env
cat .env

# Перевірити Prisma
npm run prisma:generate
```

### Проблема: сервер не запускається

```bash
# Перевірити логи PM2
pm2 logs text-rpg-api --lines 100

# Перевірити .env
cat /opt/text-rpg/server/.env | grep -E "DATABASE_URL|JWT_SECRET|PORT"

# Перевірити підключення до БД
docker ps | grep db
```

### Проблема: health check не працює

```bash
# Перевірити чи слухає порт
ss -lntp | grep :3000

# Перевірити чи працює сервер
pm2 status

# Перевірити логи
pm2 logs text-rpg-api --lines 50
```
