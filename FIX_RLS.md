# 🔧 Вирішення проблеми "Internal Server Error" при створенні персонажа

## 🔍 Проблема:

Помилка "Internal Server Error" при виклику `createCharacter` API.

## ✅ Рішення 1: Перевірити логи Backend

**Найважливіше:** Подивіться в термінал, де запущений backend сервер (`cd server && npm run dev`).

Там мають бути детальні логи помилки, які покажуть точну причину.

<hr style="border: none; border-top: 2px dotted #C9B36B; margin: 20px 0;">

## ✅ Рішення 2: RLS (Row Level Security) в Supabase

Якщо RLS увімкнений, але немає правил доступу, запити будуть блокуватися.

### Варіант A: Вимкнути RLS (для тестування)

1. Відкрити Supabase Dashboard
2. Перейти в **Table Editor**
3. Вибрати таблицю `Account`
4. Натиснути **"..."** → **"RLS"**
5. Вимкнути RLS (toggle switch)

Повторити для таблиць:
- `Account`
- `Character`
- `InventoryItem`
- `Kv`

**⚠️ Увага:** Вимкнення RLS робить дані доступними для всіх. Для production потрібно налаштувати правила RLS.

### Варіант B: Налаштувати RLS правила (рекомендовано)

Для production потрібно налаштувати правила RLS, щоб запити працювали.

<hr style="border: none; border-top: 2px dotted #C9B36B; margin: 20px 0;">

## ✅ Рішення 3: Перевірити .env файл

Переконайтеся, що `server/.env` налаштований правильно:

```env
DATABASE_URL="postgresql://postgres:ВАШ_ПАРОЛЬ@db.xxxxx.supabase.co:5432/postgres"
JWT_SECRET="your-super-secret-jwt-key-change-this-in-production"
```

<hr style="border: none; border-top: 2px dotted #C9B36B; margin: 20px 0;">

## ✅ Рішення 4: Перевірити Prisma Client

Переконайтеся, що Prisma Client згенерований:

```bash
cd server
npm run prisma:generate
```

<hr style="border: none; border-top: 2px dotted #C9B36B; margin: 20px 0;">

## 🔍 Що перевірити в логах Backend:

1. **Помилка підключення до бази даних:**
   - `Can't reach database server`
   - `Connection refused`
   - `Authentication failed`

2. **Помилка Prisma:**
   - `Prisma Client is not generated`
   - `Table does not exist`
   - `Permission denied`

3. **Помилка JWT:**
   - `JWT_SECRET is missing`
   - `Token verification failed`

<hr style="border: none; border-top: 2px dotted #C9B36B; margin: 20px 0;">

**Найпростіше рішення для тестування:** Вимкнути RLS для таблиць `Account` та `Character` в Supabase Dashboard.
