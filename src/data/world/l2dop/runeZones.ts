...
function buildRuneZoneMobs(z: { id: string; min: number; max: number }) {
  const regular = fillZoneMobs(L2DOP_RUNE_POOL, z.id, z.min, z.max, 220, 300, 12, 28).map((m, i) =>
    applyL2XmlDropsToMob(m, z.id, i)
  );
  const champions = getRuneL2DopChampions(z.id, z.min, z.max).map((m, i) => applyL2XmlDropsToMob(m, z.id, i));
