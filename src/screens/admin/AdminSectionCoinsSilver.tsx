...
import { adminFindPlayerByName, adminCoinsSilver } from "../../utils/api";

const style = { color: "#c7ad80" };

export function AdminSectionCoinsSilver() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (mode === "set") {
        ...
        const res = await adminCoinsSilver(character.id, delta);
        setMessage(`Готово. Серебряные Монеты: ${res.coinsSilver ?? "—"}`);
      }
    } catch (err: any) {
  ...
  return (
    <section className="border-t border-[#c7ad80]/30 pt-3 pb-3 first:border-t-0 first:pt-0">
      <h2 className="text-sm font-semibold mb-2" style={style}>Серебряные Монеты</h2>
      <p className="text-xs text-gray-500 mb-2">Серебряные монеты — валюта. Выдать, снять или установить.</p>
      <form onSubmit={handleSubmit} className="flex flex-wrap items-center gap-2">
        ...
        {(["give", "take", "set"] as const).map((m) => (
          <label key={m} className="flex items-center gap-0.5 text-xs" style={style}>
            <input type="radio" checked={mode === m} onChange={() => setMode(m)} className="w-3 h-3" />
            {m === "give" ? "Видати" : m === "take" ? "Забрати" : "Вст."}
          </label>
        ))}
