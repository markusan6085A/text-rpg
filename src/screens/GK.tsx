  ...
  return (
    <div className="w-full text-[#f4e2b8] px-1 py-2">
        ...
        */}
        ...
        {/* Города */}
        <div className="mb-4">
          <div className="text-[#654321] mb-2 text-xs">Города:</div>
          <div className="space-y-1">
            {WORLD_CITIES.map((city) => {
              ...
              return (
                <div
                  key={city.id}
                  className="flex items-center gap-2 text-[#654321] text-xs cursor-pointer hover:text-[#8a6342] border-b border-dotted border-gray-500 py-1"
                  onClick={() => handleCityChange(city.id)}
                >
                  <img src={iconPath} alt={city.name} className="w-3 h-3 object-contain" />
                  <span>{city.name}</span>
                  <span className="ml-auto flex items-center gap-1 text-[#654321]">
                    0
                    <img src="/assets/adena.png" alt="Adena" className="w-3 h-3 object-contain" />
                  </span>
                </div>
        ...
        {/* Локации */}
        <div className="mb-4">
          <div className="text-[#654321] mb-2 text-xs">Локации:</div>
          <div className="space-y-1">
            {zones.length === 0 ? (
              <div className="text-[#654321]/60 text-xs">
